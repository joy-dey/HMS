                ...
                fetch('modal/loginCred.php', {
                    ...
                    if (data[0] === "<") {
                        ...
                        if (data === "YES") {
                            ...
                            console.log(data);
                            alert("You are kicked out from this Hostel.. Don't Ever try to come back 🙌 !");
                            reDirect('join.html');
                            set_cookie("student", "false");
                            set_cookie("stID", "null");
                        }
...
function addStudent() {
    ...
    fetch("modal/addStudents.php", {
        ...
        if (data === "YES") {
            alert(`Hello ${dv('txtName')}, you are in the queue list. Login to your account for latest updates and management !`);
            reDirect("join.html");
        } else {
...
function initAl() {
    ...
    fetch('modal/getFreeRooms.php').then(res => { return res.json() })
        .then(data => {
            if (data[0] === "NO") {
    ...
    fetch('modal/getFreeStudents.php').then(res => { return res.json() })
        .then(data => {
            if (data[0] === "NO") {
        ...
        fetch("modal/allocateRoom.php", {
            ...
            .then(res => {
                ...
                if (data[0] === "<") {
                    ...
                    if (data === "YES") {
                        alert(temp_name + " is allocated in room number : " + elem('txtRoomNumber').value);
                        initAl();
                    }
                    else {
                        console.log(data);
...
function searchStudentsINN(btn) {
    ...
    fetch("modal/getStudents.php", {
        ...
        .then(res => {
            ...
            if (data[0] === "<") {
                elem('modalBody').innerHTML = "<p style='text-align: center;'>Error: CL (Internal Error). Please contact the administrator ! </p>"
                console.log("Error : " + data);
            }
            else {
                if (data[0] === "NO") {
                    elem('modalBody').innerHTML = "<p style='text-align: center;'>No Students with this Phone Number </p>"
                }
                else {
        ...
        fetch("modal/rfl.php", {
            ...
            .then(res => {
                ...
                if (data[0] === "<") {
                    ...
                    if (data === "YES") {
                        alert("Request Made sucessfully ! You will be notified when admin accepts the request.. Please check the room details page for updates !");
                        reDirect("studentportal.html");
                    }
                    else {
                        console.log(data);
                        alert("There was a problem in requesting. Please Try again later !");
                    }
                }
            }).catch(err => {
                alert("Internal Error. Please contact the Developers !");
